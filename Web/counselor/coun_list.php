<!DOCTYPE html>
<html lang="ko">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">

        <!-- Bootstrap core CSS -->
        <link
            href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css"
            rel="stylesheet"
            integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3"
            crossorigin="anonymous">

        <!-- Option 2: Separate Popper and Bootstrap JS -->
        <script
            src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.10.2/dist/umd/popper.min.js"
            integrity="sha384-7+zCNj/IqJ95wo16oMtfsKbZ9ccEh31eOz1HGyDuCQ6wgnyJNSYdrPa03rtR1zdB"
            crossorigin="anonymous"></script>
        <script
            src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.min.js"
            integrity="sha384-QJHtvGhmr9XOIpI6YVutG+2QOK9T+ZnN4kzFN1RtK3zEFEIsxhlmWl5/YESvpZ13"
            crossorigin="anonymous"></script>
        <link rel="stylesheet" href="../nav/nav.css">
        <link rel="stylesheet" href="counselor.css">
        <link rel="stylesheet" href="../corporation/corporation.css">
        <title>counselor</title>
    </head>

    <body>
        <?php require('../nav/nav.php'); ?>
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 wrapper">
            <div class="pt-3 pb-2 mb-3 border-bottom">
                <h3 style="text-align:left;">상담원목록</h3>
            </div>

            <div class="container-fluid">
                <div class="search">
                    
                    <div class="form-check form-check-inline" style="margin-top:5px;">
                        <input class="form-check-input" type="checkbox" id="corp_available" value="option1">
                        <label class="form-check-label" for="corp_available">사용가능</label>
                    </div>
                    <div class="form-check  form-check-inline" style="margin-top:5px;">
                        <input class="form-check-input" type="checkbox" id="corp_stop" value="option2">
                        <label class="form-check-label" for="corp_stop">정지</label>
                    </div>

                    <div class="form-group" style="float:right;">
                        <div class="input-group">
                            <button class="btn btn-outline-secondary dropdown-toggle btn-sm" type="button" data-bs-toggle="dropdown" aria-expanded="false">기업명</button>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="#">가나소프트</a></li>
                                    <li><a class="dropdown-item" href="#">알파시티</a></li>
                                </ul>
                            <input type="text" class="form-control form-control-sm" id="corp_search" placeholder="상담원 이름으로 검색">
                            <div class="input-group-append">
                                <button class="btn btn-warning btn-sm" type="submit" id="">검색</button>
                            </div>
                        </div>
                    </div>                    
                </div>
                
                <table class="table table-striped">
                    <thead class="thead-dark">
                        <tr>
                            <th scope="col">No</th>
                            <th scope="col">기업명</th>
                            <th scope="col">이름</th>
                            <th scope="col">연락처</th>
                            <th scope="col">성별</th>
                            <th scope="col">가입일</th>
                            <th scope="col">상태</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td scope="row">13</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">12</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">11</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">10</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">9</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">8</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">7</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">6</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">5</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">4</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">3</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">2</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                        <tr>
                            <td scope="row">1</td>
                            <td>가나 소프트</td>
                            <td>달봉이</td>
                            <td>010-0000-0000</td>
                            <td>남</td>
                            <td>2021-08-01</td>
                            <td>사용 가능</td>
                        </tr>
                    </tbody>
                </table>

                <div class="corp_page">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            <li class="page-item">
                            <a class="page-link" href="#" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                            </li>
                            <li class="page-item"><a class="page-link" href="#">1</a></li>
                            <li class="page-item"><a class="page-link" href="#">2</a></li>
                            <li class="page-item"><a class="page-link" href="#">3</a></li>
                            <li class="page-item">
                            <a class="page-link" href="#" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                            </li>
                        </ul>
                    </nav>
                    <button type="submit" class="btn btn-warning btn-sm px-4 new-btn">등록하기</button>
                </div>
            </div>
        </main>
    </body>

    

</html>